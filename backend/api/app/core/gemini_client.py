...
class GeminiClient:
    ...
    async def embed_content(
        ...
        """Call native Google Gemini API embedContent endpoint using the configured model."""
        ...
        url = f"https://generativelanguage.googleapis.com/v1/models/{base_model}:embedContent?key={self.api_key}"
        ...
        if not values or not isinstance(values, list):
            raise RuntimeError("No embedding values returned from Gemini")
