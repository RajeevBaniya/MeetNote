...
from app.ai import gemini_client, groq_client  # noqa: F401
from app.ai.llm_providers import get_llm_provider_class
from app.core.config import (
    AI_PROVIDER_PRIMARY,
    AI_PROVIDER_HEALTH_CHECK_INTERVAL_SECONDS,
    GEMINI_API_KEY_SUMMEREASE,
    GEMINI_MODEL_SUMMEREASE,
    GROQ_API_KEY_SUMMEREASE,
    GROQ_MODEL_SUMMEREASE,
...
class LLMGateway:
    def __init__(self) -> None:
        self.primary_name = AI_PROVIDER_PRIMARY
        self.health_check_interval = AI_PROVIDER_HEALTH_CHECK_INTERVAL_SECONDS

        primary_cls = get_llm_provider_class(self.primary_name)
        if self.primary_name == "gemini":
            self.primary_client = primary_cls(GEMINI_API_KEY_SUMMEREASE or "", GEMINI_MODEL_SUMMEREASE or "")
        elif self.primary_name == "groq":
            self.primary_client = primary_cls(GROQ_API_KEY_SUMMEREASE or "", GROQ_MODEL_SUMMEREASE or "")
        else:
        ...
        if self.fallback_name == "groq" and GROQ_API_KEY_SUMMEREASE and GROQ_MODEL_SUMMEREASE:
            fallback_cls = get_llm_provider_class("groq")
            self.fallback_client = fallback_cls(GROQ_API_KEY_SUMMEREASE, GROQ_MODEL_SUMMEREASE)
        elif self.fallback_name == "gemini" and GEMINI_API_KEY_SUMMEREASE and GEMINI_MODEL_SUMMEREASE:
            fallback_cls = get_llm_provider_class("gemini")
            self.fallback_client = fallback_cls(GEMINI_API_KEY_SUMMEREASE, GEMINI_MODEL_SUMMEREASE)
    ...
    async def generate_content(
        ...
        if self.primary_healthy:
            for attempt in range(SUMMEREASE_MAX_RETRIES):
                try:
                    if attempt == 0:
                        await asyncio.sleep(SUMMEREASE_THROTTLE_DELAY)
                    else:
                        backoff = 2 ** attempt
                        logger.warning(
                    ...
                    if not self._is_transient_error(exc):
                        logger.error("primary_provider_permanent_failure provider=%s err=%s", self.primary_name, exc)
                        raise
                    logger.warning(
        ...
        if self.fallback_client:
            ...
            try:
                ...
                logger.error("fallback_provider_failed provider=%s err=%s", self.fallback_name, exc)
                raise
        else:
            raise RuntimeError(
